...
THIS IS TRYING TO FIGURE OUT THE LENGTH OF THE LONGEST STRINGS IN EACH AND THEIR POSITIONS
    
    max_len = tmp = pos1 = pos2 = 0
    len1, len2 = len(str1), len(str2)

start at 0 with lengths of sytrings 1 and 2

    for p in range(len1):
        for every start point in len1
        for q in range(len2):
        start at q in len2
            tmp = 0
